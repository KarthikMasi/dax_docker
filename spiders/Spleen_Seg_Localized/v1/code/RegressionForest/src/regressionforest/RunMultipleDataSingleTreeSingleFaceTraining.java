...
public class RunMultipleDataSingleTreeSingleFaceTraining {
	public static void main(String[] args) throws Exception {
		if (args.length != 5) {
			...
			System.out.println("5. index of face (boundary)");
			return;
		}		
		// arguments
		...
		DataCollection dc = new DataCollection(classCount, dimCount, indexB);
		...
		for (int i = 0; i < imgFiles.length; i++){
			...
			float[][] box = BoundaryFinder.GetBoundry(label, value, voxres);
			float [][] boxload = new float [classCount][1];
			for (int c = 0; c < classCount; ++c)
				boxload[c][0] = box[c][indexB];
			dc.AddDataItem(img, boxload, voxres);
		}
