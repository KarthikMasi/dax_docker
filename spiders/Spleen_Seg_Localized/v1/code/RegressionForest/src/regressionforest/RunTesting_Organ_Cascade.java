...
public class RunTesting_Organ_Cascade {
	public static void main(String[] args) throws Exception {
		...
		for (int c = 0; c < classCount; c++){
			...
			int leftVox = (int) ((float)(box_global[c][1] + pad) / voxres[0]);
			if (leftVox >= xx) leftVox = xx - 1;
			int backVox = (int) ((float)(box_global[c][2] - pad) / voxres[1]);
			if (backVox < 0) backVox = 0;
			int frontVox = (int) ((float)(box_global[c][3] + pad) / voxres[1]);
			if (frontVox >= yy) frontVox = yy - 1;
			int buttomVox = (int) ((float)(box_global[c][4] - pad) / voxres[2]);
			if (buttomVox < 0) buttomVox = 0;
			int topVox = (int) ((float)(box_global[c][5] + pad) / voxres[2]);
			...
			box[c][1] = box_organ[0][1] + (double)((float)rightVox * voxres[0]);
			box[c][2] = box_organ[0][2] + (double)((float)backVox * voxres[1]);
			box[c][3] = box_organ[0][3] + (double)((float)backVox * voxres[1]);
			box[c][4] = box_organ[0][4] + (double)((float)buttomVox * voxres[2]);
			box[c][5] = box_organ[0][5] + (double)((float)buttomVox * voxres[2]);		
		}
