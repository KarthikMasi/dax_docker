...
public class RunTesting_Organ_CascadeInBatch {
	public static void main(String[] args) throws Exception {
		...
		for (int c = 0; c < classCount; c++){
			...
			for (int ii = 0; ii < imgFiles.length; ii++){
				...
				int leftVox = (int) ((float)(box_global[ii][c][1] + pad) / voxres[0]);
				if (leftVox >= xx[ii]) leftVox = xx[ii] - 1;
				int backVox = (int) ((float)(box_global[ii][c][2] - pad) / voxres[1]);
				if (backVox < 0) backVox = 0;
				int frontVox = (int) ((float)(box_global[ii][c][3] + pad) / voxres[1]);
				if (frontVox >= yy[ii]) frontVox = yy[ii] - 1;
				int buttomVox = (int) ((float)(box_global[ii][c][4] - pad) / voxres[2]);
				if (buttomVox < 0) buttomVox = 0;
				int topVox = (int) ((float)(box_global[ii][c][5] + pad) / voxres[2]);
				...
				box_organ[ii][c][1] = box_tmp[0][1] + (double)((float)rightVox * voxres[0]);
				box_organ[ii][c][2] = box_tmp[0][2] + (double)((float)backVox * voxres[1]);
				box_organ[ii][c][3] = box_tmp[0][3] + (double)((float)backVox * voxres[1]);
				box_organ[ii][c][4] = box_tmp[0][4] + (double)((float)buttomVox * voxres[2]);
				box_organ[ii][c][5] = box_tmp[0][5] + (double)((float)buttomVox * voxres[2]);
			}
		}
		System.out.println("Training cost: " + Double.toString((double)(System.currentTimeMillis() -  tBeginOrgan) / 1000) + " seconds");
		System.out.println("saving result ...");
		for (int ii = 0; ii < imgFiles.length; ii++){
			String fn = imgFiles[ii].substring(imgFiles[ii].indexOf("img")+3, imgFiles[ii].indexOf(".nii.gz"));
