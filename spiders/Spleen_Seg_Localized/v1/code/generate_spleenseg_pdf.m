function generate_spleenseg_pdf(rawfn, segfn, pdffn, cmap_mat,...
    tmp_dir,proj_name, subj_name, expr_name, scan_name, spider_name)
...
citation_line1=['Z. Xu et al. ',...
    '"Efficient multi-atlas abdominal segmentation on ',...
    'clinically acquired CT with '];
citation_line2=['SIMPLE context learning" ',...
    'Medical Image Analysis, August 2015.'];

indent=0.02;
t_projsubjexprscan_label=text(indent,0.85,...
    'Project/Subject/Experiment/Scan: ',...
    'fontsize',9,'fontweight','bold',...
    'interpreter','none');
et=get(t_projsubjexprscan_label,'extent');
ns=et(1)+et(3)+1*indent;
t_projsubjexprscan_content=text(ns,0.85,...
    sprintf('%s/%s/%s/%s',...
    proj_name,subj_name,expr_name,scan_name),...
    'fontsize',9,...
    'interpreter','none');

t_spidername_label=text(indent,0.70,...
    'Spider name: ',...
    'fontsize',9,'fontweight','bold',...
    'interpreter','none');
et=get(t_spidername_label,'extent');    
ns=et(1)+et(3)+1*indent;
t_spidername_content=text(ns,0.70,...
    spider_name,...
    'fontsize',9,...
    'interpreter','none');

t_citation_line1_label=text(indent,0.55,...
    'Citation: ',...
    'fontsize',9,'fontweight','bold',...
    'interpreter','none');
et=get(t_citation_line1_label,'extent');
ns=et(1)+et(3)+1*indent;
t_citation_line1_content=text(ns,0.55,...
    citation_line1,...
    'fontsize',9,...
    'interpreter','none');
t_citation_line2_content=text(indent,0.45,...
    citation_line2,...
    'fontsize',9,...
    'interpreter','none');

t_email_label=text(indent,0.30,...
    'Contact: ',...
    'fontsize',9,'fontweight','bold',...
    'interpreter','none');
et=get(t_email_label,'extent');
ns=et(1)+et(3)+1*indent;
t_email_content=text(ns,0.30,...
    email,...
    'fontsize',9,...
    'interpreter','none');

t_rundate_label=text(indent,0.15,...
    'Date of run: ',...
    'fontsize',9,'fontweight','bold',...
    'interpreter','none');
et=get(t_rundate_label,'extent');
ns=et(1)+et(3)+1*indent;
t_rundate_content=text(ns,0.15,...
    rundate(1:end-1),...
    'fontsize',9,...
    'interpreter','none');
